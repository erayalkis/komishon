...
fn walk_and_save(base_dir: &str) {
    ...
    for entry in WalkDir::new(base_dir) {
        ...
        let query = conn.prepare(base_statement).expect("Error while preparing SQL statement");

        let path = entry_path.extension().expect("Error while reading path extension").to_str();
...
fn main() {
    let app = tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![greet, return_map, walk_and_save])
